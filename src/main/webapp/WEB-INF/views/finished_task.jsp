...
<html lang="en">
  ...
  <body>
        ...
        <div class="col-md-4" th:each="task: ${taskListResponse.tasks}">          <div class="p-card bg-white p-2 rounded px-3" th:classappend="${task.isCompleted} ? 'task-completed' : ''">
            ...
            >
              ...
              <a
                ...
                th:attr="onclick='confirmDelete(event, ' + ${task.id} + ', \'' + ${task.title} + '\', ' + ${isFinishedTasksPage} + ', \'' + ${sortBy} + '\')'"
                title="Delete task"
              >
            ...
            <div class="d-flex justify-content-between stats">
              <div>
                ...
                <a
                th:href="@{/finish_task/{id}(id=${task.id}, sortBy=${sortBy}, keyword=${searchKeyword}, fromFinishedPage=${isFinishedTasksPage})}"
    ...
    ></script>
    <script>
      function confirmDelete(event, taskId, taskTitle, isFinishedPage, sortBy) {
        event.preventDefault();
        if (
          confirm('Bạn có chắc muốn xóa công việc "' + taskTitle + '" không?')
        ) {
          let url = "/delete_task/" + taskId;
          url += "?fromFinishedPage=" + isFinishedPage;
          if (sortBy) {
            url += "&sortBy=" + sortBy;
          }
